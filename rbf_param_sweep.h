...
struct output_files {
  ...
  output_files(const std::string &name) {
    ...
    auto op_header = [width = width + 1](auto &out) {
      out << std::setw(width) << "N " << std::setw(width) << "sigma "
          << std::setw(width) << "D " << std::setw(width) << "order "
          << std::setw(width) << "Nsubdomain " << std::setw(width) << "matrix "
          << std::setw(width) << "dense " << std::setw(width) << "fmm "
          << std::setw(width) << "h2 " << std::endl;
    };
    ...
    for (int i = 0; i < Nops; ++i) {
      out_solve_setup_time[i].open(name + op_names[i] + "solve_setup_time.txt",
                                   std::ios::out);
      solve_header(out_solve_setup_time[i]);
      out_solve_solve_time[i].open(name + op_names[i] + "solve_solve_time.txt",
                                   std::ios::out);
      ...
      out_solve_iterations[i].open(name + op_names[i] + "solve_iterations.txt",
                                   std::ios::out);
      solve_header(out_solve_iterations[i]);
      out_solve_error[i].open(name + op_names[i] + "solve_error.txt",
                              std::ios::out);
      solve_header(out_solve_error[i]);
      out_solve_test_error[i].open(name + op_names[i] + "solve_test_error.txt",
                                   std::ios::out);
...
void helper_operator_matrix(Op &G, const TrueOp &Gtrue,
                            ...
                            const Eigen::VectorXd &phi_test, output_files &out,
                            bool do_solve) {
...
void helper_param_sweep(const Particles_t &particles, const size_t Ntest,
  ...
  helper_operator_matrix(Gmatrix.get_first_kernel().get_matrix(), Gmatrix, phi,
                         Gmatrix_test, phi_test, out, true);
  ...
  if (D < 4) {
    ...
    t0 = Clock::now();
    auto G_H2 =
        create_h2_operator(knots, knots, Order, kernel, self_kernel, eta, beta);
    t1 = Clock::now();
    out.out_op_setup_time
        << " " << std::setw(out.width)
        << std::chrono::duration_cast<std::chrono::milliseconds>(t1 - t0)
    ...
    auto G_H2_test =
        create_h2_operator(test, knots, Order, kernel, self_kernel, eta, beta);
